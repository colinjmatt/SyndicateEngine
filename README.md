...
- `E`: in first-mission control mode, queue debug-gated interaction/action intents for selected original-agent markers at the current original cursor tile; if the last route was blocked by a candidate door, open that runtime-only route gate locally, while final door animation/lock/gameplay semantics remain gated.
