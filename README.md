...
- Left click: attack in the demo city; in first-mission control mode, select a nearby original-agent marker or have the selected original-agent set fire at a clicked current-objective/non-squad ped candidate using gated local combat state, runtime weapon/loadout hints, and a temporary shot/status overlay.
