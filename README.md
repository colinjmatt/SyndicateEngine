...
- Left click: attack in the demo city; in first-mission control mode, select a nearby original-agent marker or have the selected original-agent set fire at a clicked current-objective/non-squad ped candidate using gated local combat state and a temporary shot/status overlay.
...
5. Replace provisional local combat/objective state with proved FreeSynd-style weapon config, blocker scans, scenario/objective mutation, AI, door, pickup, vehicle-entry, and final mission-completion semantics as each behavior is proven.
