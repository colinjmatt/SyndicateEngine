...
- `E`: in first-mission control mode, queue debug-gated interaction/action intents for selected original-agent markers at the current original cursor tile; local door/open resolutions can enable a runtime-only route gate overlay, while final door semantics remain gated.
