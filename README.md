...
- `E`: in first-mission control mode, queue debug-gated interaction/action intents for selected original-agent markers at the current original cursor tile; if the last route was blocked by a candidate door, selected agents can approach the closed-door threshold, the route shows a local door-closed label, and `E` opens that runtime-only route gate locally before immediately retrying the stored movement goal, while final door animation/lock/gameplay semantics remain gated.
