...
impl OriginalMissionScene {
    ...
    pub fn interaction_objective_report_label(&self) -> String {
        format!(
            "{}; {}; {}; {}; {}; debug action resolution gate ready for local control labels only; static scene report remains aggregate-only; gated world runtime may attach local movement/combat/objective lifecycle, formation-spaced route orders with fallback diagnostics and wider destination-tail reservations, staggered squad starts, cancellable selected local orders, route-blocked door hints with visible local overlays and runtime-only door-open route gates that can immediately retry stored movement goals, local down/defeated dynamic route blockers, active command selection excluding local down-test agents, selected-agent camera follow/reset, compact normal tactical HUD with per-agent HP/cooldown bars and decoder diagnostics hidden until trace, aligned route probe labels, firing-position cooldown holds, facing/impact/volley/down labels, persistent door-open route/vehicle/pickup-proof overlays, civilian panic/flee markers, hostile return-fire plus route-gated hostile/civilian pressure markers, verified automated local mission-complete/final-state/reset/interaction-gate playtest traces, and dropped-pickup blockers without mutating source GAME data, doors, inventory, vehicles, AI, or final mission results",
