...
- BLK-like graphics containers such as `MMAPBLK.DAT` and `HBLK01.DAT` are now inspected with non-reconstructable aggregate diagnostics: RNC status, decoded length, byte entropy/zero/unique summaries, plausible fixed-size indexed-pixel record counts, and range-only correlations against MAP substrate candidate byte lanes. These rows can show that a MAP candidate range could address a block/tile candidate record count, but they do not prove terrain, building, object, or render-layout semantics.
